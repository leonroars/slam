...
public class ConcertSchedule {
    ...
    // ID 포함 정적 팩토리 메서드.
    public static ConcertSchedule create(Long id, Long concertId, LocalDateTime dateTime,
    ...
    // ID 미포함 정적 팩토리 메서드.
    public static ConcertSchedule create(Long concertId, LocalDateTime dateTime, LocalDateTime reservationStartAt,
    ...
    public static ConcertSchedule create(Long concertId, LocalDateTime dateTime, LocalDateTime reservationStartAt,
            LocalDateTime reservationEndAt, ConcertScheduleAvailability availability) {

        evaluateConcertSchedule(dateTime, reservationStartAt, reservationEndAt);

        ConcertSchedule concertSchedule = new ConcertSchedule();
        concertSchedule.id = TSID.fast().toLong();
    ...
    public static ConcertSchedule create(Long concertScheduleId, Long concertId, LocalDateTime dateTime,
            LocalDateTime reservationStartAt, LocalDateTime reservationEndAt,
     ...
     * <br>
     * </br>
     * 성립 불가한 공연 일정, 예약 시작 일자, 예약 종료 일자가 아닌 지 검증하고 성립 불가할 시
     ...
     */
    private static void evaluateConcertSchedule(LocalDateTime dateTime, LocalDateTime reservationStartAt,
            LocalDateTime reservationEndAt) {
        if (reservationStartAt.isAfter(reservationEndAt)) {
            throw new BusinessRuleViolationException(ErrorCode.INVALID_RESERVATION_PERIOD,
                    "예약 시작 일자는 예약 종료 일자보다 늦을 수 없습니다.");
        }
        if (dateTime.isBefore(reservationStartAt)) {
            throw new BusinessRuleViolationException(ErrorCode.INVALID_CONCERT_DATE_BEFORE_RESERVATION,
                    "공연 일자가 예약 가능 시작 일자에 선행합니다.");
        }
        if (dateTime.isBefore(reservationEndAt)) {
            throw new BusinessRuleViolationException(ErrorCode.INVALID_CONCERT_DATE_DURING_RESERVATION,
                    "예약 가능 기간 중엔 공연이 종료될 수 없습니다.");
        }
    }
