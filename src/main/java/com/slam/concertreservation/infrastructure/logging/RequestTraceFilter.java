...
public class RequestTraceFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        try {
            ...
            String matchedPattern = (String) httpRequest.getAttribute(HandlerMapping.BEST_MATCHING_PATTERN_ATTRIBUTE);
            String uriForLog = (matchedPattern == null) ? httpRequest.getRequestURI() : matchedPattern;

            // 8. 처리 시간 계산
            long duration = System.currentTimeMillis() - startTime;
            MDC.put("duration", String.valueOf(duration));
            MDC.put("status", String.valueOf(httpResponse.getStatus()));
            MDC.put("uri", uriForLog);
